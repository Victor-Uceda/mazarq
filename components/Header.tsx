...
/** Navigation bar – glass‑morphism pill that shrinks on scroll,
 *  with a desktop nav and a full‑screen mobile hamburger menu. */
const links = [
...
export default function Header() {
  ...
  /* scroll‑driven shrink + atmosphere progress on desktop */
  useEffect(() => {
      ...
      <div className="mx-auto max-w-[1760px]">
        <div ref={headerRef} className="header-glass relative flex items-center rounded-full px-6 py-3.5 sm:px-8">
          ...
          <div className="flex flex-1 items-center gap-3 sm:gap-3">
            <a href="#" aria-label="Inicio" className="flex items-center gap-2.5 sm:gap-3">
              ...
              <span className="leading-tight">
                <span className="block text-[9px] font-semibold uppercase tracking-[0.28em] text-black/45 sm:text-[10px] sm:tracking-[0.38em]">mazarq</span>
                <span className="mt-0.5 hidden text-sm text-black/62 sm:block">3D Modeler / Arch Viz Enthusiast</span>
              </span>
            </a>
          </div>
          ...
          <nav className="hidden items-center gap-8 text-[10px] font-semibold uppercase tracking-[0.22em] text-black/45 md:flex">
            {links.map((link) => (
              <a key={link.href} href={link.href} onClick={() => setMenuOpen(false)} className="transition hover:text-black">
                {link.label}
              </a>
            ))}
          </nav>
          ...
          <div className="flex flex-1 items-center justify-end gap-3">
            <a href="#contacto" className="liquid-button hidden rounded-full px-5 py-2.5 text-[10px] font-bold uppercase tracking-[0.22em] text-black sm:inline-flex">
              Contratar
            </a>
        ...
        <AnimatePresence>
          {menuOpen && (
            <>
              ...
              >
                <button
                  type="button" onClick={() => setMenuOpen(false)}
