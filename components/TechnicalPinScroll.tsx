...
/** Pin‑scroll technical showcase – 3D model board with scroll‑driven
 *  rotation and frame progression. Renders static on mobile. */
const frames = [
  { eyebrow: "Axonometria explotada",    title: "Analisis constructivo",    text: "Capas, estructura, cerramientos y lectura tecnica para explicar como se arma una propuesta." },
  { eyebrow: "Refugio-Taller",           title: "Arquitectura ligera",      text: "Volumen, sombra, vegetacion y escala humana para comunicar una idea con pocas lineas." },
  { eyebrow: "Arch Viz",                 title: "Presentacion comercial",   text: "Imagenes listas para vender, sustentar decisiones y acelerar aprobaciones." }
];
  ...
  return (
    <div className="mx-auto grid h-full w-full max-w-[1760px] gap-8 lg:grid-cols-[1.08fr_.92fr] lg:items-center">
      ...
      <div ref={boardRef} className="technical-board relative h-[45dvh] overflow-hidden rounded-[2.4rem] border border-black/10 bg-white shadow-[0_35px_110px_rgba(20,26,32,.08)] sm:h-[min(68vh,46rem)] sm:rounded-[3.5rem]">
        ...
        {[0, 1, 2, 3, 4].map((item) => (
          <div key={item} className="technical-callout"
            style={{ top: `${18 + item * 13}%`, left: `${8 + item * 3}%`, opacity: active >= Math.min(item, 2) ? 1 : 0.25 }}>
      ...
      <div className="relative z-10">
        <p className="text-[10px] font-semibold uppercase tracking-[0.42em] text-black/42">{frames[active].eyebrow}</p>
        <h2 className="display mt-5 text-[clamp(2.8rem,8vw,9rem)] leading-[0.78] text-[#111] sm:text-[clamp(4rem,8vw,9rem)]">
          {frames[active].title}
        </h2>
        <p className="mt-6 text-base leading-7 text-black/58 sm:mt-7 sm:text-lg sm:leading-8">{frames[active].text}</p>
...
export default function TechnicalPinScroll() {
  ...
  useEffect(() => {
    ...
    return subscribe((scrollY, viewportH) => {
      ...
      else if (rect.bottom <= viewportH) setPinState("after");
      else setPinState("active");

      if (barRef.current) barRef.current.style.width = `${Math.max(next * 100, 7)}%`;
      if (modelRef.current) {
  ...
  /* Mobile – render as a static section */
  if (isMobile) {
    return (
      <section className="relative z-10 bg-[#f8f8f5] px-5 py-24 sm:px-8 lg:px-12">
        <TechnicalContent active={active} modelRef={modelRef} barRef={barRef} boardRef={boardRef} />
      </section>
    );
  }

  const pinClass =
    pinState === "active" ? "fixed inset-0 z-20"
